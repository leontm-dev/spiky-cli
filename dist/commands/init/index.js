...
export default async function initCommand(str, options) {
    ...
    let overwrite = true;
    process.stdout.write(`${chalk.yellowBright.italic(formSteps({ current: 2, max: maxSteps }))} Moving to step 2`);
    if (config.found) {
        updateOldConsole(`${chalk.yellowBright.italic(formSteps({ current: 2, max: maxSteps }))} Waiting for your input...`);
        console.log("");
        ...
        if (!answer) {
            updateOldConsole(`${chalk.greenBright.italic(formSteps({ current: 2, max: maxSteps }))} Not overwriting the current config.spiky.json file. Skipping step 3`);
            overwrite = answer;
