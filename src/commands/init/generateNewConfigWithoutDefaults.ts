...
export default async function generateNewConfigWithoutDefault(
  ...
  try {
      ...
      .prompt([
        {
          ...
          required: true,
          message:
            "Please specify the input file in which your are writing your typescript spiky.js code.",
        },
        {
          type: "select",
          ...
          choices: [
            {
              ...
              value: "project",
              description:
                "Should the build command update the code inside of your project directly?",
              separator: ",",
            },
            {
              name: "File-Based",
              value: "python",
              description:
                "Should the build command update a specified file that is formatted if needed?",
              separator: ",",
            },
          ],
        },
      ...
      const answer = await inquirer
        .prompt([
          {
            ...
            required: true,
            message:
              "Please provide the path to the folder of the exisiting spiky prime project. This folder has to be unzipped. If the folder doesn't exist, a new project will be created.",
          },
        ])
        ...
        .catch((err) => {
          updateOldConsole(
            `${chalk.redBright.italic(
              formSteps(functionIndexes)
            )} Unable to finish process properly.`
          );
          throw err;
        });
      ...
      const answers = await inquirer
        .prompt([
          ...
          },
          {
            ...
            message:
              "Choose how your code should look like after being transpiled",
          },
        ])
        ...
        .catch((err) => {
          updateOldConsole(
            `${chalk.redBright.italic(
              formSteps(functionIndexes)
            )} We could not finish the process.`
          );
          throw err;
        });
    ...
    updateOldConsole(
      `${chalk.greenBright.italic(
        formSteps(functionIndexes)
      )} Created/Updated config.spiky.json`
    );
    return true;
  } catch (error) {
    updateOldConsole(
      `${chalk.redBright.italic(
        formSteps(functionIndexes)
      )} We couldn't complete the process successfully.`
    );
    return false;
  }
}
